...
import { subscribeToSchedule, updateEventStatus, initializeDefaultData, getThemeStats, getCommunityStats } from '../services/firestoreService';
...
const AnalyticsDashboard = ({ stats = {}, isLive = false }) => {
    ...
    const aggregatedSlides = useMemo(() => {
        if (isLive && stats.topTopics) {
            return stats.topTopics.map(topic => ({
                id: topic.name,
                title: topic.name,
                lights: topic.count,
                type: 'slide'
...
const Backstage = () => {
    ...
    const [viewMode, setViewMode] = useState('scheduler'); // 'scheduler' | 'analytics'
    ...
    // Fetch stats
    useEffect(() => {
        if (isAuthenticated && viewMode === 'analytics' && !communityStats) {
            setStatsError(null);
            getCommunityStats()
            ...
            <div style={{
                ...
                <div className="container" style={{ maxWidth: '800px', margin: '0 auto', padding: '0 1rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                        <button
                            onClick={() => setViewMode(viewMode === 'scheduler' ? 'analytics' : 'scheduler')}
                            style={{
                                border: 'none',
                                cursor: 'pointer',
                                color: '#0052CC',
                                background: viewMode === 'analytics' ? '#dbeafe' : '#e6f0ff',
                                padding: '0.4rem 1rem',
                                borderRadius: '4px',
                                fontSize: '0.8rem',
                                fontWeight: '600'
                            }}>
                            {viewMode === 'scheduler' ? '📊 Ver Analíticas' : '📅 Ver Calendario'}
                        </button>
                        <button
                ...
                <div className="container" style={{ maxWidth: '800px', margin: '0 auto', padding: '2rem 1rem' }}>
                    ...
                    {statsError ? (
                        <div style={{ padding: '2rem', backgroundColor: '#FEF2F2', border: '1px solid #FECACA', borderRadius: '8px', color: '#991B1B' }}>
                            ...
                            <p style={{ marginTop: '0.5rem', fontSize: '0.8rem', opacity: 0.8 }}>Si el error persiste, verifica la consola del navegador.</p>
                        </div>
                    ) : (
